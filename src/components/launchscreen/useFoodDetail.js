...
const modifyPassioFoodItemByCheckSelectedUnitExist = passioFoodItem => {
...
export const useFoodDetail = prop => {
  ...
  function extractFoodNutrients(passioNutrients) {
  ...
  const onAddIngredients = item => {
    ...
    setPassioFoodItem(foodItem => {
      ...
      foodItem.amount.selectedQuantity = newQuantity
      foodItem.name = 'Recipe with ' + foodItem.name.replace('Recipe with ', '')
